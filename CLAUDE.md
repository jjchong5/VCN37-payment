...
- `render.yaml` — Render blueprint matching that split: `x402-book-seller` (Node web service running `server.js`, needs `PAY_TO_ADDRESS` set in Render's dashboard) and `x402-book-demo` (static site publishing `./site`). Not yet deployed as of this writing — `site/app.js`'s default seller URL assumes the seller service exists at that Render URL once deployed.
